...
q_boots <- function(fx, x, m, nsim=999,
  ...
  set.seed(seedinit)
  bootsamp <- rsample::bootstraps(as.data.frame(as.factor(Y)),
                                  times = nsim)
  Qfull_boot <- purrr::map(bootsamp$splits,
                           q_symb_A2, ms, symb)
  Qfull_stat <- unlist(Qfull_boot)
  Qpboot <- Qfull_stat[names(Qfull_stat) == "qp"]
  Qcboot <- Qfull_stat[names(Qfull_stat) == "qc"]
  ...
  for (i in 1:nsim) {
    mefp_symb[,i] <- Qfull_boot[[i]]$efp_symb
    mefc_symb[,i] <- Qfull_boot[[i]]$efc_symb
  }
  pvalueboot_p <- sum(Qpboot > Q0$qp) / (nsim + 1)
  pvalueboot_c <- sum(Qcboot > Q0$qc) / (nsim + 1)
  results <- list(Q0$qp, pvalueboot_p,
                  Q0$qc, pvalueboot_c,
                  Q0$qp_symb, Q0$qc_symb,
                  Q0$PSymb, Q0$CSymb,
                  Q0$efp_symb, Q0$efc_symb,
                  Qpboot, Qcboot, mefp_symb, mefc_symb,
                  ms, mdtms, symb, distance)
  names(results) <- c("qp", "pvalueboot_qp",
                      "qc", "pvalueboot_qc",
                      "qp_symb", "qc_symb",
                      "PSymb", "CSymb",
                      "efp_symb", "efc_symb",
                      "qpboot", "qcboot",
                      "efp_symb_boot", "efc_symb_boot",
                      "ms", "mdtms", "symb", "distance")
  return(results)
}
