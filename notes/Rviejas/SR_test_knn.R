...
#' @usage SR_test2(fx=fx,listw=listw,alternative="two.sided")
...
#' srq <- SRQ_test_knn(fx = QY, listw = listw)
#' srq$SRglobal
#'
#' SRQ test based on inverse distance
#' data("FastFood")
#' x <- cbind(FastFood.sf$Lon,FastFood.sf$Lat)
#' dis <- geosphere::distm(x,x,fun=distCosine)
#'
#' dis <- sf::st_distance(FastFood.sf)
#'
#' # Load dataset
#' data("FastFood")
#' # Define coordinates
#' x <- cbind(FastFood.sf$Lon,FastFood.sf$Lat)
#' listw <- knearneigh(x, k=4)
#' qruns <- SR_test_knn(FastFood.sf$Type,listw)
#'
#' #' # Load dataset
#' data("Newark")
#' # Define coordinates
#' x <- cbind(Newark$X,Newark$Y)
#' listw <- knearneigh(x, k=10)
#' qruns <- SR_test_knn(Newark$Type,listw)
#'

SRQ_test_knn <-  function(fx = fx,listw = listw, alternative="two.sided" , nsim = NULL){

  if (class(listw) != "knn"){
  stop ("tiene que ser de la clase knn")
}
...
############################################################
# Salida
return <- list(SR=SR,dnr=dnr,SRglobal=SRQ,p.valueSRQ=p.valueSRQ, SRLP=SRLP,SRGP=SRGP,SRQlocal=SRQlocal,MeanNeig=sum(lnnb)/n,listw=listw)
}
