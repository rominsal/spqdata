...
#' @usage Q.map.test(formula = formula, data = data, coor = NULL, m = m, r = 1, type = "combinations", control = list())
#' @keywords map comparison, spatial association, qualitative variable, symbolic entropy, symbols, independence
...
#' }
#'
#' @details Aquí Antonio escribe una linda historia ... \cr
#' Two alternative sets of arguments can be included in this function to compute the spatial runs test:
#'
#'   \tabular{ll}{
#'     \code{Alternative 1} \tab A factor (xf) and a list of neighborhood (\code{listw}) of the class knn. \cr
#'     \code{Alternative 2} \tab A sf object (data) and formula to specify the factor. A list of neighborhood (listw) \cr
#'     }
#'
...
#'     \code{gl} \tab free degree of the statistic for QE or QI\cr
#'     \code{mh} \tab m-surrounding of th map.\cr
#'     \code{Tm} \tab number of maps (ONLY 2). \cr
#'     \code{R} \tab  number of symbolized observations. \cr
#'     \code{nsk} \tab a matrix Tm x  symbols with the frequency of the number
#'      of symbols of each map \cr
#'     }
#' @author
#'   \tabular{ll}{
...
#'   }
#' @seealso
#' \code{\link{dgp.spq}}, \code{\link{m.surround}}
#' @export
#' @examples
#'
#' # Case 1:
...
Q.map.test <- function(formula = formula, data = data, coor = NULL, m = m, r = 1, type = "combinations", control = list()) {
  ...
  if (("sf" %in% class(data)) && is.null(coor)){
    coor <- suppressWarnings(sf::st_centroid(sf::st_geometry(data)))
    coor <- st_as_sf(coor)
  } else if (!("sf" %in% class(data)) && is.matrix(coor)){
  ...
  p.value.Equivalence = pchisq(QE, df = gl.Equivalence, lower.tail = FALSE)

  QI <- 2*R*(-sum(rowSums(fnk*lfnk))+sum(FNK*LFNK))
  gl.Independence <- nusi*(nusi-2) + 1
  p.value.Independence = pchisq(QI, df = gl.Independence, lower.tail = FALSE)
