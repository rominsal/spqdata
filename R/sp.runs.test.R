...
#' @param distr distribution of the test "asymptotic" (default) or "bootstrap"
...
#' @usage sp.runs.test(formula = NULL, data = NULL, na.action, xf = NULL,
...
#'     \code{nb} \tab Si los vecinos se obtiene a partir de un objeto sf, el código internamente llamará
...
sp.runs.test <-  function(formula = NULL, data = NULL, na.action, xf = NULL,
...
data.name <- names(mxf)
if ((is.null(nsim) == FALSE) && (distr != "asymptotic")){
srq <- list(method = "Run test of spatial dependence for qualitative data. Boots version",
...
{
srq <- list(method = "Run test of spatial dependence for qualitative data. Asymptotic version",
