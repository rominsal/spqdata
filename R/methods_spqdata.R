...
plot.spqtest <- function(object, ci = 0.95, viewplot = TRUE, ...) {
  ...
  for (i in 1:length(z)) {
    zi <- z[[i]]
    R <- zi$R
    if (zi$type == "permutations") {
      qsymb <- zi$qp_symb
      efsymb <- zi$efp_symb
      Symb <- zi$PSymb
      efsymb_boot <- zi$efp_symb_boot
    } else if (zi$type == "combinations") {
      qsymb <- zi$qc_symb
      efsymb <- zi$efc_symb
      Symb <- zi$CSymb
      efsymb_boot <- zi$efc_symb_boot
    } else stop("type must be permutations or combinations")
    ...
    } else if (zi$distr == "bootstrap") {
      for (j in 1:length(sf0)) {
        lb_int[j] <- quantile(efsymb_boot[j,],
                                alpha_div_2)
        ub_int[j] <- quantile(efsymb_boot[j,],
                               1 - alpha_div_2)
        }
      lb_int <- lb_int / R
      ub_int <- ub_int / R
    } else stop("distribution must be asymptotic or bootstrap")
